...
import { LinkOutlined, PlusOutlined, SaveOutlined, SettingOutlined } from '@ant-design/icons';
...
const Header = memo(({ onSettingsClick }) => {
  ...
  useEffect(() => {
    if (isSuccess && data && shouldFetch) {
      if (data.length === 0) {
        message.info('No unsolved problems found or all problems are already solved!');
      } else {
  ...
  const handleSyncToLocalStorage = () => {
    try {
      ...
      message.error('Failed to sync to Local Storage');
    }
  };
